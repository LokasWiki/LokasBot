...
class Query:
    def __init__(self, db_name):
        ...
        self.cursor.execute(
            '''CREATE TABLE IF NOT EXISTS requests (id INTEGER PRIMARY KEY, page_name TEXT, target_page TEXT, status INTEGER)''')
        self.connection.commit()

    def insert_request(self, request):
        self.cursor.execute(
            '''INSERT INTO requests(page_name, target_page, status) VALUES(?,?,?)''', request)
        self.connection.commit()

    def pop_request(self):
        self.cursor.execute('''SELECT * FROM requests WHERE status = 0 ORDER BY id LIMIT 1''')
        request = self.cursor.fetchone()
        if request:
            self.cursor.execute('''UPDATE requests SET status = 1 WHERE id = ?''', (request[0],))
            self.connection.commit()
            return request
        return None

    def count_requests(self):
        self.cursor.execute('''SELECT COUNT(*) FROM requests WHERE status = 0''')
